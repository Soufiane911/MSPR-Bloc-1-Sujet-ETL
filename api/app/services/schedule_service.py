...
class ScheduleService:
    ...
    def get_schedules(
        ...
        query = """
            SELECT 
                ...
                t.train_type,
                so.name as origin,
                COALESCE(so.city, so.name) as origin_city,
                COALESCE(so.country, 'EU') as origin_country,
                sd.name as destination,
                COALESCE(sd.city, sd.name) as destination_city,
    ...
    def search_schedules(
        ...
        query = """
            ...
            JOIN stations so ON s.origin_id = so.station_id
            JOIN stations sd ON s.destination_id = sd.station_id
            WHERE (so.city ILIKE :from_city OR so.name ILIKE :from_city)
            AND (sd.city ILIKE :to_city OR sd.name ILIKE :to_city)
        """
        params = {
