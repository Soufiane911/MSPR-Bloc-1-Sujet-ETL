...
class DataNormalizer:
    ...
    def normalize_station_codes(
        self, df: pd.DataFrame, code_column: str = "stop_id"
    ) -> pd.DataFrame:
        """
        ...
        if code_column in df.columns:
            ...
            self.logger.info(
                f"[OK] Codes normalises: {df[code_column].nunique()} codes uniques"
            )
    ...
    def normalize_time(self, time_str: str) -> Optional[time]:
        ...
        try:
            ...
            self.logger.warning(f"[WARN] Format de temps invalide: {time_str} - {e}")
    ...
    def normalize_stop_times(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in ["arrival_time", "departure_time"]:
            if col in df.columns:
                ...
                if errors > 0:
                    self.logger.warning(
                        f"[WARN] {errors} valeurs de temps invalides dans {col}"
                    )
    ...
    def add_country_code(
        ...
        self.logger.info(f"[OK] Code pays ajoute: {source_country}")
    ...
    # Types ferroviaires à conserver (selon spec GTFS)
    RAIL_ROUTE_TYPES = {
        2: "Rail",
        100: "Railway Service",
        101: "High Speed Rail",
    ...
    # Tous les types GTFS (pour l'information)
    ALL_ROUTE_TYPES = {
        **RAIL_ROUTE_TYPES,
        0: "Tram",
        1: "Subway",
    ...
    def standardize_route_types(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Returns:
            pd.DataFrame: Types standardisés avec labels
        """
        df = df.copy()
        ...
        if "route_type" in df.columns:
            ...
            df["route_type_label"] = (
                df["route_type"].map(self.ALL_ROUTE_TYPES).fillna("Unknown")
            )

        return df

    def filter_rail_routes_only(self, routes_df: pd.DataFrame) -> pd.DataFrame:
        """
        Filtre pour ne garder que les routes ferroviaires.

        CRITIQUE: Réduit drastiquement le volume de données (ex: CFF/SBB passe
        de 2.2M à 245k trips, soit -89% de données inutiles).

        Args:
            routes_df: DataFrame des routes (doit contenir route_type)

        Returns:
            pd.DataFrame: Routes ferroviaires uniquement
        """
        if routes_df.empty or "route_type" not in routes_df.columns:
            return routes_df

        initial_count = len(routes_df)

        # Convertir en numérique pour le filtrage
        routes_df = routes_df.copy()
        routes_df["route_type"] = pd.to_numeric(
            routes_df["route_type"], errors="coerce"
        )

        # Garder uniquement les types ferroviaires
        rail_types = list(self.RAIL_ROUTE_TYPES.keys())
        filtered = routes_df[routes_df["route_type"].isin(rail_types)].copy()
        ...
        if removed_count > 0:
            self.logger.info(
                f"[FILTER] Routes filtrées: {removed_count}/{initial_count} "
                f"({removal_pct:.1f}%) - Bus/Tram/Non-rail supprimés"
            )
            # Afficher la répartition des types supprimés
            removed = routes_df[~routes_df["route_type"].isin(rail_types)]
            if not removed.empty:
                type_counts = removed["route_type"].value_counts().head(5)
                self.logger.info(f"[FILTER] Types supprimés: {type_counts.to_dict()}")
        else:
            self.logger.info(
                f"[FILTER] Toutes les routes sont ferroviaires ({initial_count} routes)"
            )
