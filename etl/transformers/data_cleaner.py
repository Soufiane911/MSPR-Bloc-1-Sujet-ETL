...
class DataCleaner:
    ...
    def remove_duplicates(self, df: pd.DataFrame, subset: Optional[List[str]] = None) -> pd.DataFrame:
        ...
        if removed > 0:
            self.logger.info(f"✓ Doublons supprimés: {removed} lignes")
    ...
    def handle_missing_values(self, df: pd.DataFrame, 
        ...
        if total_missing > 0:
            self.logger.warning(f"⚠ Valeurs manquantes détectées:")
        ...
        if strategy == 'drop':
            ...
            if removed > 0:
                self.logger.info(f"✓ Lignes supprimées (valeurs manquantes): {removed}")
    ...
    def clean_stations(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'stop_lat' in df.columns and 'stop_lon' in df.columns:
            ...
            if invalid_coords.sum() > 0:
                self.logger.warning(f"⚠ {invalid_coords.sum()} coordonnées invalides")
                df.loc[invalid_coords, ['stop_lat', 'stop_lon']] = np.nan
        
        self.logger.info(f"✓ Gares nettoyées: {len(df)} lignes")
    ...
    def clean_routes(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        self.logger.info(f"✓ Lignes nettoyées: {len(df)} lignes")
    ...
    def clean_stop_times(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        self.logger.info(f"✓ Horaires nettoyés: {len(df)} lignes")
